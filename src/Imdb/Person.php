    ...
    protected function filmograf(&$res, $type)
    {
        ...
        if (!empty($match) && preg_match_all('!<div class="filmo-row.*?>\s*(.*?)\s*</div!ims', $match[1], $matches)) {
            ...
            for ($i = 0; $i < $mc; ++$i) {
                $year = '';
                $type = Title::MOVIE;
                if (!preg_match('!href="/title/tt(\d{8}|\d{7})/[^"]*"\s*>(.*?)</a>\s*</b>\n?(.*)!ims', $matches[1][$i],
                  $mov)) {
                    continue;
                }
                $char = array();
                ...
                if (preg_match('!href="/character/ch(\d{8}|\d{7})[^"]*"\s*>(.*?)</a>!ims', $matches[1][$i], $char)) {
                    $chid = $char[1];
                    $chname = $char[2];
                } else {
                    $chid = '';
        ...
        if (empty($this->bio_salary)) {
            ...
            if (preg_match_all("/<tr.*?<td.*?>(.*?)<\/td>.*?<td.*?>(.*?)<\/td>/ms", $block,
              $matches)) { // for each table row
                $mc = count($matches[0]);
                for ($i = 0; $i < $mc; ++$i) {
                    if (preg_match("/\/title\/tt(\d{8}|\d{7})\/\">(.*?)<\/a>\s*\((\d{4})\)/", $matches[1][$i], $match)) {
                        $movie["imdb"] = $match[1];
                        $movie["name"] = $match[2];
                        $movie["year"] = $match[3];
                    } else {
